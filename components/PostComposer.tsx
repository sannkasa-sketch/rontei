...
export function PostComposer({ slug, factions, primaryFactionId, allowFactionSelection, relationType = "main", parentPostId = null, compact = false, onCancel, onSuccess }: { slug: string; factions: MyTopicFaction[]; primaryFactionId: string; allowFactionSelection: boolean; relationType?: CreatePostRelation; parentPostId?: string | null; compact?: boolean; onCancel?: () => void; onSuccess?: () => void }) {
  ...
  const effectiveFactionId = factions.some((faction) => faction.faction_id === factionId) ? factionId : initialFactionId;
  const selectedFaction = factions.find((faction) => faction.faction_id === effectiveFactionId);
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await createDebatePost(slug, content, parentPostId, relationType, effectiveFactionId);
  ...
  return <form onSubmit={submit} className={compact ? "mt-3 rounded-lg border border-slate-200 bg-slate-50 p-4" : ""}>
    ...
    <div className="mt-2 flex items-center justify-between gap-3"><span className={`text-xs ${content.length >= 4800 ? "font-bold text-orange-700" : "text-slate-400"}`}>{content.length} / 5000</span><div className="flex gap-2">{onCancel && <button type="button" onClick={onCancel} disabled={pending} className="button-secondary !min-h-9 !px-3 !py-2">キャンセル</button>}<button type="submit" data-testid={relationType === "main" ? "main-post-submit" : `reply-submit-${relationType}`} disabled={pending || content.trim().length === 0 || !effectiveFactionId} className="button-primary !min-h-9 !px-4 !py-2 disabled:cursor-not-allowed disabled:opacity-60">{pending ? "送信中…" : compact ? "返信する" : "発言する"}</button></div></div>
