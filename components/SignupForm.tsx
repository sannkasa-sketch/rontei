...
export function SignupForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (data.session && data.user) {
      const { error: profileError } = await supabase.from("profiles").update({ account_name: accountName }).eq("id", data.user.id);
      if (profileError) {
        setError(getSignupError(profileError.message));
        setPending(false);
        return;
      }
      router.push("/mypage");
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
      <div><label htmlFor="signup-account-name" className="block text-sm font-bold text-slate-700">アカウント名</label><input id="signup-account-name" name="accountName" autoComplete="nickname" aria-invalid={Boolean(fieldErrors.accountName)} aria-describedby="signup-account-name-help signup-account-name-error" className={inputClass} /><p id="signup-account-name-help" className="mt-1.5 text-xs leading-5 text-slate-500">完全記名の討論ではこの名前が表示されます。</p>{fieldErrors.accountName && <p id="signup-account-name-error" role="alert" className="mt-1.5 text-sm font-semibold text-rose-700">{fieldErrors.accountName}</p>}</div>
